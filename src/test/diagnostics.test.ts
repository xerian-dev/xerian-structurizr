...
suite('Diagnostics', () => {
	test('valid DSL file produces no diagnostics', async () => {
		const doc = await openDslDocument(path.join(sampleDir, 'example.dsl'));
		const diags = await waitForDiagnostics(doc.uri, 0);
		assert.strictEqual(diags.length, 0, 'Expected 0 diagnostics for valid file');
	});
	...
	test('reports unterminated string', async () => {
		...
		const stringDiag = diags.find(d => d.message.includes('Unterminated string'));
